...
'$libdir/plugins/plugin_debugger', 'plpgsql_oid_debug'
  ...
  COST 1;
ALTER FUNCTION plpgsql_oid_debug(oid)
  OWNER TO postgres;

-- for backwards-compatibility
--CREATE FUNCTION plpgsql_oid_debug( functionOID OID ) RETURNS INTEGER AS $$ SELECT pldbg_oid_debug($1) $$ LANGUAGE sql STRICT;

CREATE FUNCTION pldbg_oid_debug( functionOID OID ) RETURNS INTEGER AS $$ SELECT plpgsql_oid_debug($1) $$ LANGUAGE sql STRICT;

CREATE OR REPLACE FUNCTION pldbg_abort_target(session integer)
  RETURNS SETOF boolean AS
'$libdir/pldbgapi', 'pldbg_abort_target'
  LANGUAGE c VOLATILE STRICT
  COST 1
  ROWS 1000;
ALTER FUNCTION pldbg_abort_target(integer)
  OWNER TO postgres;
  
CREATE OR REPLACE FUNCTION pldbg_attach_to_port(portnumber integer)
  RETURNS integer AS
'$libdir/pldbgapi', 'pldbg_attach_to_port'
  LANGUAGE c VOLATILE STRICT
  COST 1;
ALTER FUNCTION pldbg_attach_to_port(integer)
  OWNER TO postgres;
