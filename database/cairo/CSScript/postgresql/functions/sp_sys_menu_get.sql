...
begin
   ...
   where m.me_object_handler not in ('cUsuarioPermiso','cRolPermiso')
      or (s.sysm_objetoedicion = 'CSOAPIEdit.cUsuarioPermiso'
           and s.sysm_id = -8 and s.pre_id = 8
           and m.me_object_handler = 'cUsuarioPermiso')
      or (s.sysm_objetoedicion = 'CSOAPIEdit.cRolPermiso'
           and s.sysm_id = 8 and s.pre_id = 8
           and m.me_object_handler = 'cRolPermiso')
   order by
