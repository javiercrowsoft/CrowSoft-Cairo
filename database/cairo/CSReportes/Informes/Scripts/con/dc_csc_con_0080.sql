   ...
   open rtn for
      ...
      select fc.fc_id comp_id,
             ...
             case fc.doct_id
                ...
                when 10 then fcperc_importe   end Importe,
             case fc.doct_id
                            when 2 then fc_neto
                            when 8 then -fcperc_importe
                            when 10 then fc_neto   end Neto,
             case fc.doct_id
                            when 2 then fc_ivari + fc_ivarni
                            when 8 then -fc_ivari + fc_ivarni
                            when 10 then fc_ivari + fc_ivarni   end IVA,
             case fc.doct_id
                            when 2 then fc_total
                            when 8 then -fc_total
                            when 10 then fc_total   end Total
        from FacturaCompra fc
               join FacturaCompraPercepcion fcperc
                on fc.fc_id = fcperc.fc_id
               join Percepcion perc
                on fcperc.perc_id = perc.perc_id
               join PercepcionTipo perct
                on perc.perct_id = perct.perct_id
               join Documento doc
                on fc.doc_id = doc.doc_id
               join Empresa emp
                on doc.emp_id = emp.emp_id
               join CircuitoContable cico
                on doc.cico_id = cico.cico_id
               join Proveedor prov
...
/////////////////////////////////////////////////////////////////////// */
                 and ( perct.cue_id = v_cue_id
                 or v_cue_id = 0 )
                 and ( fc.ccos_id = v_ccos_id
                 or v_ccos_id = 0 )
                 and ( doc.cico_id = v_cico_id
                 or v_cico_id = 0 )
                 and ( fc.prov_id = v_prov_id
                 or v_prov_id = 0 )
                 and ( fc.doc_id = v_doc_id
                 or v_doc_id = 0 )
                 and ( fcperc.perc_id = v_perc_id
                 or v_perc_id = 0 )
                 and ( perc.perct_id = v_perct_id
                 or v_perct_id = 0 )
      ...
      select opg.opg_id comp_id,
                             ...
                             ( select max(fc_nrodoc)
                               from FacturaCompra fc
                                      join FacturaCompraOrdenPago fcp
        ...
        from OrdenPago opg
               ...
               join Retencion ret
                on opgi.ret_id = ret.ret_id
               join RetencionTipo rett
                on ret.rett_id = rett.rett_id
               join Documento doc
...
INICIO SEGUNDA PARTE DE ARBOLES
/////////////////////////////////////////////////////////////////////// */
                 and ( opgi.cue_id = v_cue_id
                 or v_cue_id = 0 )
                 and ( opg.ccos_id = v_ccos_id
                 or v_ccos_id = 0 )
                 and ( doc.cico_id = v_cico_id
                 or v_cico_id = 0 )
                 and ( opg.prov_id = v_prov_id
                 or v_prov_id = 0 )
                 and ( opg.doc_id = v_doc_id
                 or v_doc_id = 0 )
                 and ( opgi.ret_id = v_ret_id
                 or v_ret_id = 0 )
                 and ( ret.rett_id = v_rett_id
                 or v_rett_id = 0 )
                 and ( emp.emp_id = v_emp_id
                 or v_emp_id = 0 )
                 -- Arboles
      ...
      select fv.fv_id comp_id,
                             ...
                             case fv.doct_id
                                            ...
                                            when 9 then fvperc_importe   end Importe,
                             case fv.doct_id
                                            when 1 then fv_neto
                                            when 7 then -fvperc_importe
                                            when 9 then fv_neto   end Neto,
                             case fv.doct_id
                                            when 1 then fv_ivari + fv_ivarni
                                            when 7 then -fv_ivari + fv_ivarni
                                            when 9 then fv_ivari + fv_ivarni   end IVA,
                             case fv.doct_id
                                            when 1 then fv_total
                                            when 7 then -fv_total
                                            when 9 then fv_total   end Total
        from FacturaVenta fv
               join FacturaVentaPercepcion fvperc
                on fv.fv_id = fvperc.fv_id
               join Percepcion perc
                on fvperc.perc_id = perc.perc_id
               join PercepcionTipo perct
                on perc.perct_id = perct.perct_id
               join Documento doc
...
/////////////////////////////////////////////////////////////////////// */
                 and ( perct.cue_id = v_cue_id
                 or v_cue_id = 0 )
                 and ( fv.ccos_id = v_ccos_id
                 or v_ccos_id = 0 )
                 and ( doc.cico_id = v_cico_id
                 or v_cico_id = 0 )
                 and ( fv.cli_id = v_cli_id
                 or v_cli_id = 0 )
                 and ( fv.doc_id = v_doc_id
                 or v_doc_id = 0 )
                 and ( fvperc.perc_id = v_perc_id
                 or v_perc_id = 0 )
                 and ( perc.perct_id = v_perct_id
                 or v_perct_id = 0 )
                 and ( emp.emp_id = v_emp_id
      ...
      select cobz.cobz_id comp_id,
        ...
        from Cobranza cobz
               join CobranzaItem cobzi
                on cobz.cobz_id = cobzi.cobz_id
               and cobzi_tipo = 4--Otros

               join Retencion ret
                on cobzi.ret_id = ret.ret_id
               join RetencionTipo rett
                on ret.rett_id = rett.rett_id
               join Documento doc
                on cobz.doc_id = doc.doc_id
               join Empresa emp
                on doc.emp_id = emp.emp_id
               join CircuitoContable cico
                on doc.cico_id = cico.cico_id
...
INICIO SEGUNDA PARTE DE ARBOLES
/////////////////////////////////////////////////////////////////////// */
                 and ( cobzi.cue_id = v_cue_id
                 or v_cue_id = 0 )
                 and ( cobz.ccos_id = v_ccos_id
                 or v_ccos_id = 0 )
                 and ( doc.cico_id = v_cico_id
                 or v_cico_id = 0 )
                 and ( cobz.cli_id = v_cli_id
                 or v_cli_id = 0 )
                 and ( cobz.doc_id = v_doc_id
                 or v_doc_id = 0 )
                 and ( cobzi.ret_id = v_ret_id
                 or v_ret_id = 0 )
                 and ( ret.rett_id = v_rett_id
                 or v_rett_id = 0 )
                 and ( emp.emp_id = v_emp_id
...
$BODY$
  ...
  cost 100;
alter function dc_csc_con_0080(integer, date, date, varchar, varchar, integer)
  owner to postgres;