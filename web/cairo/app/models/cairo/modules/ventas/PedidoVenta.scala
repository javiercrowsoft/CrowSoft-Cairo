...
object PedidoVenta {
  ...
  private val pedidoVentaParser: RowParser[PedidoVenta] = {
    ...
    SqlParser.get[String](C.RAM_ID_STOCK) ~
  ...
  private def save(user: CompanyUser, pedidoVenta: PedidoVenta, isFromWizard: Boolean): PedidoVenta = {
    def getFields = {
      List(
        ...
        Field(C.RAM_ID_STOCK, pedidoVenta.stock.ramIdStock, FieldType.text),
    ...
    def executeSave(pvTMPId: Int): List[RowResult] = {
      ...
      DB.withTransaction(user.database.database) { implicit connection =>
        ...
        try {
          ...
          * the last kind of type is key. in this case the key must be the name of a column like pv_id, as_id, pr_id, etc
  ...
  def delete(user: CompanyUser, id: Int) = {
    DB.withConnection(user.database.database) { implicit connection =>
      try {
        SQL("select * from sp_doc_pedido_venta_delete({id}, {empId}, {usId})")
