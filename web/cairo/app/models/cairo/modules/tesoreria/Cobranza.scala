...
object Cobranza {
  ...
  private def save(user: CompanyUser, cobranza: Cobranza): Cobranza = {
    ...
    def getTarjetaFields(item: CobranzaItemTarjeta, cobzTMPId: Int) = {
      List(
        ...
        Field(C.COBZI_TMP_CUPON, item.cuponNumeroDoc, FieldType.text),
        Field(GC.TJC_ID, item.tjcId, FieldType.id),
        Field(GC.TJCCU_ID, item.tjccuId, FieldType.id),
        Field(GC.MON_ID, item.moneda.monId, FieldType.id),
        Field(C.COBZI_TMP_FECHA_VTO, item.fechaVto, FieldType.date),
        Field(C.COBZI_TMP_TITULAR, item.titular, FieldType.text),
        Field(C.COBZI_TMP_NRO_TARJETA, item.numero, FieldType.text),
        Field(C.COBZI_TMP_AUTORIZACION, item.autorizacion, FieldType.text),
        Field(C.COBZI_TARJETA_TIPO, item.tarjetaTipo, FieldType.id),
    ...
    def getOtroFields(item: CobranzaItemOtro, cobzTMPId: Int) = {
      List(
        ...
        Field(C.COBZI_NRO_RETENCION, item.retencion.numero, FieldType.text),
        Field(C.COBZI_PORC_RETENCION, item.retencion.porcentaje, FieldType.currency),
        Field(C.COBZI_FECHA_RETENCION, item.retencion.fecha, FieldType.date),
        Field(C.FV_ID_RET, item.retencion.fvId, FieldType.id),
        Field(GC.CCOS_ID, item.base.ccosId, FieldType.id),
        Field(C.COBZI_ORDEN, item.base.orden, FieldType.integer),
        Field(C.COBZI_IMPORTE, item.totals.importe, FieldType.currency),
        Field(C.COBZI_IMPORTE_ORIGEN, item.totals.importeOrigen, FieldType.currency)
  ...
  def delete(user: CompanyUser, id: Int) = {
    DB.withConnection(user.database.database) { implicit connection =>
      try {
        SQL("select * from sp_doc_cobranza_delete({id}, {empId}, {usId})")
