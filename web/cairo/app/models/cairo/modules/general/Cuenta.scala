  ...
  private def save(user: CompanyUser, cuenta: Cuenta, isNew: Boolean): Cuenta = {
    def getFields = {
      List(
        ...
        Field(C.CUE_LLEVA_CENTRO_COSTO, Register.boolToInt(cuenta.llevaCentroCosto), FieldType.boolean),
        Field(C.CUE_PRODUCTO, Register.boolToInt(cuenta.producto), FieldType.boolean),
        Field(C.CUE_ES_EFECTIVO, Register.boolToInt(cuenta.esEfectivo), FieldType.boolean),
        Field(C.CUE_ES_TICKET, Register.boolToInt(cuenta.esTicket), FieldType.boolean),
