...
//*TODO:** type is translated as a new class at the end of the file Private Type T_Aplic
            ...
            var pSaveDocCpra = function(docId,  rcTMPId) { // TODO: Use of ByRef founded Private Function pSaveDocCpra(ByVal DocId As Long, ByRef RcTMPId As Long) As Boolean
                ...
                register.getFields().add2(mComprasConstantes.RC_NUMERO, 0, Cairo.Constants.Types.long);
                register.getFields().add2(mComprasConstantes.RC_NRODOC, "", Cairo.Constants.Types.text);
            ...
            var pItLoadItemsAux = function(iProp,  isOrden) { // TODO: Use of ByRef founded Private Function pItLoadItemsAux(ByRef iProp As cIABMProperty, ByVal IsOrden As Boolean) As Boolean
                ...
                sqlstmt = "sp_DocRemitoCompraGetAplic "+ cUtil.getEmpId().toString()+ ","+ m_rcId+ ",1";
            ...
            var pItLoadAplicAplicados = function(vAplic,  isOrden) { // TODO: Use of ByRef founded Private Function pItLoadAplicAplicados(ByRef vAplic() As T_Aplic, ByVal IsOrden As Boolean) As Boolean
                ...
                if(isOrden) {
                    sqlstmt = "sp_DocRemitoCompraGetAplic "+ cUtil.getEmpId().toString()+ ","+ m_rcId+ ",4";
                }
                else {
                    sqlstmt = "sp_DocRemitoCompraGetAplic "+ cUtil.getEmpId().toString()+ ","+ m_rcId+ ",2";
                }
            ...
            var pItLoadAplicCreditos = function(vAplic,  isOrden) { // TODO: Use of ByRef founded Private Function pItLoadAplicCreditos(ByRef vAplic() As T_Aplic, ByVal IsOrden As Boolean) As Boolean
                ...
                if(isOrden) {
                    sqlstmt = "sp_DocRemitoCompraGetAplic "+ cUtil.getEmpId().toString()+ ","+ m_rcId+ ",5";
                }
                else {
                    sqlstmt = "sp_DocRemitoCompraGetAplic "+ cUtil.getEmpId().toString()+ ","+ m_rcId+ ",3";
                }
                            ...
                            if(vAplic(i).vAplicaciones(j).rci_id !== Cairo.Constants.NO_ID) {
                                ...
                                if(vAplic.Aplicado > 0) {
                                    ...
                                    if(vAplic(i).fci_id !== 0) {
                                        ...
                                        register.getFields().add2(mComprasConstantes.RC_FC_CANTIDAD, vAplic.Aplicado, Cairo.Constants.Types.double);
                                        register.getFields().add2(mComprasConstantes.RC_FC_ID, vAplic.vinc_id, Cairo.Constants.Types.long);
                                        ...
                                        if(m_isDevolucion) {
                                            register.getFields().add2(mComprasConstantes.RCI_ID_DEVOLUCION, vAplic.rci_id, Cairo.Constants.Types.id);
                                            register.getFields().add2(mComprasConstantes.RCI_ID_REMITO, vAplic(i).rci_id, Cairo.Constants.Types.id);
                                        }
                                        else {
                                            register.getFields().add2(mComprasConstantes.RCI_ID_REMITO, vAplic.rci_id, Cairo.Constants.Types.id);
                                            register.getFields().add2(mComprasConstantes.RCI_ID_DEVOLUCION, vAplic(i).rci_id, Cairo.Constants.Types.id);
                                        }

                                        register.getFields().add2(mComprasConstantes.RC_DC_CANTIDAD, vAplic.Aplicado, Cairo.Constants.Types.double);
                                        register.getFields().add2(mComprasConstantes.RC_DC_ID, vAplic.vinc_id, Cairo.Constants.Types.long);
