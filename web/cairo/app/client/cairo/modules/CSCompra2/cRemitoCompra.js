(function() {
  ...
  Cairo.module("XxxxListDoc.Edit", function(Edit, Cairo, Backbone, Marionette, $, _) {
    ...
    var createObject = function() {
      ...
      // cRemitoCompraListDoc
      // 24-01-04

      var C_MODULE = "cRemitoCompraListDoc";
      ...
      var load = function(us_id) {
        ...
        return Cairo.Database.getData("load[" + apiPath + "general/remitocompralistdoc]", id).then(
          function(response) {
            ...
            if(response.data.id === Cairo.Constants.NO_ID) {
              ...
              strLoad = Cairo.Language.getText(2165, "");
              //Error al cargar los párametros de navegación de Remitos de Compra
              ...
              if(m_prov_id.Substring(0, 1).toUpperCase() != KEY_NODO) {
                if(!Cairo.Database.getData(mComprasConstantes.PROVEEDOR, mComprasConstantes.PROV_ID, Cairo.Util.val(m_prov_id), mComprasConstantes.PROV_NAME, data, C_LoadFunction, C_MODULE, strLoad)) { return false; }
                m_proveedor = data;
              }
              if(m_est_id.Substring(0, 1).toUpperCase() != KEY_NODO) {
              ...
              if(m_cpg_id.Substring(0, 1).toUpperCase() != KEY_NODO) {
                if(!Cairo.Database.getData(mComprasConstantes.CONDICIONPAGO, mComprasConstantes.CPG_ID, Cairo.Util.val(m_cpg_id), mComprasConstantes.CPG_NAME, data, C_LoadFunction, C_MODULE, strLoad)) { return false; }
      ...
      var cIABMListDocClient_Refresh = function() {
        ...
        sqlstmt = "sp_lsdoc_RemitosCompra ";
      ...
      var cIABMListDocClient_Save = function() {
        ...
        strError = Cairo.Language.getText(2165, "");
        ...
        sqlstmt = "delete ListaDocumentoParametro  where pre_id ="+ csComprasPrestacion.cSPRECPRALISTREMITO.toString()+ " and us_id = "+ m_us_id+ " and (emp_id is null or emp_id = "+ cUtil.getEmpId().toString()+ ")";
        ...
        for (var _i = 0; _i < _count; _i++) {
          ...
          register.getFields().add2(Cairo.Constants.PRE_ID, csComprasPrestacion.cSPRECPRALISTREMITO, Cairo.Constants.Types.id);
      ...
      var pShowMensajes = function() {
        ...
        sqlstmt = "sp_ParteDiarioGetTitleForDoc "+ csEDocumentoTipo.cSEDT_REMITOCOMPRA.toString()+ ","+ rcId.toString();
        ...
        sqlstmt = "sp_PartesDiarioGetForDoc "+ Cairo.Database.getUserId().toString()+ ","+ cUtil.getEmpId().toString()+ ","+ doctId.toString()+ ","+ rcId.toString();
        ShowNotes(Cairo.Language.getText(1951, "", infodoc), sqlstmt);
        //Notas sobre Remito  & infodoc, sqlstmt
      };
      ...
      var pAddMensaje = function() {
        ...
        parte.AddParteToDoc(csEDocumentoTipo.cSEDT_REMITOCOMPRA, m_objList.Id, false);
      };
      ...
      var pFirmar = function() {
        ...
        if(rcId == Cairo.Constants.NO_ID) { return; }

        var firmado = null;
        var docId = null;

        if(!Cairo.Database.getData(mComprasConstantes.REMITOCOMPRA, mComprasConstantes.RC_ID, rcId, mComprasConstantes.RC_FIRMADO, firmado)) { return; }
        if(!Cairo.Database.getData(mComprasConstantes.REMITOCOMPRA, mComprasConstantes.RC_ID, rcId, mComprasConstantes.DOC_ID, docId)) { return; }
      ...
      var pShowDocAux = function() {
        ...
        if(rcId) {
          ...
          var stId = null;
          if(!Cairo.Database.getData(mComprasConstantes.REMITOCOMPRA, mComprasConstantes.RC_ID, rcId, mComprasConstantes.ST_ID, stId)) { return; }
      ...
      var pShowApply = function() {
        ...
        sqlstmt = "select rc_total, rc_cotizacion, rc_nrodoc, rc.prov_id, prov_nombre, rc.suc_id, rc.doc_id, rc.doct_id from RemitoCompra rc inner join Proveedor prov  on rc.prov_id = prov.prov_id where rc_id = "+ rcId.toString();

        if(!Cairo.Database.openRs(sqlstmt, rs)) { return; }

        if(rs.isEOF()) { return; }

        total = Cairo.Database.valField(rs.getFields(), mComprasConstantes.RC_TOTAL);
        cotiz = Cairo.Database.valField(rs.getFields(), mComprasConstantes.RC_COTIZACION);
        nroDoc = Cairo.Database.valField(rs.getFields(), mComprasConstantes.RC_NRODOC);
        provId = Cairo.Database.valField(rs.getFields(), mComprasConstantes.PROV_ID);
        proveedor = Cairo.Database.valField(rs.getFields(), mComprasConstantes.PROV_NAME);
      ...
      var pGetProvId = function() {
        ...
        rcId = m_objList.Id;
        Cairo.Database.getData(mComprasConstantes.REMITOCOMPRA, mComprasConstantes.RC_ID, rcId, mComprasConstantes.PROV_ID, provId);

        return provId;
      };

      var pGetRcIds = function() {
      ...
      self.initialize = function() {
        try {
          ...
          //'Remitos de Compras
          m_title = Cairo.Language.getText(1944, "");
