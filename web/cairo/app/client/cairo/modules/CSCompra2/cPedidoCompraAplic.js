...
//*TODO:** type is translated as a new class at the end of the file Private Type T_Aplic
            ...
            self.show = function(pcId,  total,  pcNumero,  usId,  usuario,  sucId,  docId,  isDevolucion) {
                ...
                if(m_pcId !== pcId) {
                    ...
                    // Edit Apply
                    //
                    if(!Cairo.Database.getData(mComprasConstantes.DOCUMENTO, mComprasConstantes.DOC_ID, m_docId, Cairo.Constants.EMP_ID, m_emp_id)) { return _rtn; }
            ...
            var pSaveDocCpra = function(docId,  pcTMPId) { // TODO: Use of ByRef founded Private Function pSaveDocCpra(ByVal DocId As Long, ByRef pcTMPId As Long) As Boolean
                ...
                register.getFields().add2(mComprasConstantes.PC_ID, m_pcId, Cairo.Constants.Types.id);

                register.getFields().add2(mComprasConstantes.PC_NUMERO, 0, Cairo.Constants.Types.long);
                register.getFields().add2(mComprasConstantes.PC_NRODOC, "", Cairo.Constants.Types.text);

                register.getFields().add2(Cairo.Constants.US_ID, 0, Cairo.Constants.Types.long);
            ...
            var pItLoadItemsAux = function(iProp) { // TODO: Use of ByRef founded Private Function pItLoadItemsAux(ByRef iProp As cIABMProperty) As Boolean
                ...
                sqlstmt = "sp_DocPedidoCompraGetAplic "+ cUtil.getEmpId().toString()+ ","+ m_pcId+ ",1";
            ...
            var pItLoadAplicAplicados = function(vAplic) { // TODO: Use of ByRef founded Private Function pItLoadAplicAplicados(ByRef vAplic() As T_Aplic) As Boolean
                ...
                sqlstmt = "sp_DocPedidoCompraGetAplic "+ cUtil.getEmpId().toString()+ ","+ m_pcId+ ",2";
            ...
            var pItLoadAplicCreditos = function(vAplic) { // TODO: Use of ByRef founded Private Function pItLoadAplicCreditos(ByRef vAplic() As T_Aplic) As Boolean
                ...
                sqlstmt = "sp_DocPedidoCompraGetAplic "+ cUtil.getEmpId().toString()+ ","+ m_pcId+ ",3";
                            ...
                            if(vAplic(i).vAplicaciones(j).pci_id !== Cairo.Constants.NO_ID) {
                                ...
                                if(vAplic.Aplicado > 0) {
                                    ...
                                    if(vAplic(i).oci_id !== 0) {
                                        ...
                                        register.getFields().add2(mComprasConstantes.PCCOT_CANTIDAD, vAplic.Aplicado, Cairo.Constants.Types.double);
                                        register.getFields().add2(mComprasConstantes.PCCOT_ID, vAplic.vinc_id, Cairo.Constants.Types.long);
                                        ...
                                        if(m_isDevolucion) {
                                            register.getFields().add2(mComprasConstantes.PCI_ID_DEVOLUCION, vAplic.pci_id, Cairo.Constants.Types.id);
                                            register.getFields().add2(mComprasConstantes.PCI_ID_PEDIDO, vAplic(i).pci_id, Cairo.Constants.Types.id);
                                        }
                                        else {
                                            register.getFields().add2(mComprasConstantes.PCI_ID_PEDIDO, vAplic.pci_id, Cairo.Constants.Types.id);
                                            register.getFields().add2(mComprasConstantes.PCI_ID_DEVOLUCION, vAplic(i).pci_id, Cairo.Constants.Types.id);
                                        }

                                        register.getFields().add2(mComprasConstantes.PC_DC_CANTIDAD, vAplic.Aplicado, Cairo.Constants.Types.double);
                                        register.getFields().add2(mComprasConstantes.PC_DC_ID, vAplic.vinc_id, Cairo.Constants.Types.long);
