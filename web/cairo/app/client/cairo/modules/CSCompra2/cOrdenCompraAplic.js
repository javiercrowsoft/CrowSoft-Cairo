...
private class T_ItAplic {
    public Long vinc_id;//' lo uso para ocrc_id, ocfc_id y ocdc_id
    public Long oci_id;//' Ids de items al que este credito esta vinculado
    public Double aplicado;
}


private class T_Aplic {
    public Long pci_id;//' Vinculacion con pedido
    public Long rci_id;//' remito
    public Long fci_id;//' factura
    public Long oci_id;//' cancelacion de orden / orden
    public Long pR_ID;
    public Date fecha;
...
case class OrdencompraaplicData(
    id: Option[Int],

)

object Ordencompraaplics extends Controller with ProvidesUser {

    val ordencompraaplicForm = Form(
        mapping(
                "id" -> optional(number),

        )(OrdencompraaplicData.apply)(OrdencompraaplicData.unapply))

    implicit val ordencompraaplicWrites = new Writes[Ordencompraaplic] {
        def writes(ordencompraaplic: Ordencompraaplic) = Json.obj(
                "id" -> Json.toJson(ordencompraaplic.id),
                C.ID -> Json.toJson(ordencompraaplic.id),

        )
    }

    def get(id: Int) = GetAction { implicit request =>
        LoggedIntoCompanyResponse.getAction(request, CairoSecurity.hasPermissionTo(S.LIST_ORDENCOMPRAAPLIC), { user =>
        Ok(Json.toJson(Ordencompraaplic.get(user, id)))
    })
}

def update(id: Int) = PostAction { implicit request =>
    Logger.debug("in ordencompraaplics.update")
    ordencompraaplicForm.bindFromRequest.fold(
        formWithErrors => {
        Logger.debug(s"invalid form: ${formWithErrors.toString}")
    BadRequest
},
ordencompraaplic => {
    Logger.debug(s"form: ${ordencompraaplic.toString}")
    LoggedIntoCompanyResponse.getAction(request, CairoSecurity.hasPermissionTo(S.EDIT_ORDENCOMPRAAPLIC), { user =>
    Ok(
        Json.toJson(
            Ordencompraaplic.update(user,
                Ordencompraaplic(
                    id,
...
def create = PostAction { implicit request =>
    Logger.debug("in ordencompraaplics.create")
    ordencompraaplicForm.bindFromRequest.fold(
        formWithErrors => {
        Logger.debug(s"invalid form: ${formWithErrors.toString}")
    BadRequest
...
ordencompraaplic => {
    Logger.debug(s"form: ${ordencompraaplic.toString}")
    LoggedIntoCompanyResponse.getAction(request, CairoSecurity.hasPermissionTo(S.NEW_ORDENCOMPRAAPLIC), { user =>
    Ok(
        Json.toJson(
            Ordencompraaplic.create(user,
                Ordencompraaplic(
...
def delete(id: Int) = PostAction { implicit request =>
    Logger.debug("in ordencompraaplics.delete")
    LoggedIntoCompanyResponse.getAction(request, CairoSecurity.hasPermissionTo(S.DELETE_ORDENCOMPRAAPLIC), { user =>
    Ordencompraaplic.delete(user, id)
...
object Ordencompraaplic {

    lazy val emptyOrdencompraaplic = Ordencompraaplic(
    )
    ...
    private val ordencompraaplicParser: RowParser[Ordencompraaplic] = {
...
def create(user: CompanyUser, ordencompraaplic: Ordencompraaplic): Ordencompraaplic = {
    save(user, ordencompraaplic, true)
}

def update(user: CompanyUser, ordencompraaplic: Ordencompraaplic): Ordencompraaplic = {
    save(user, ordencompraaplic, false)
}

private def save(user: CompanyUser, ordencompraaplic: Ordencompraaplic, isNew: Boolean): Ordencompraaplic = {
    def getFields = {
        List(
    ...
    def throwException = {
        throw new RuntimeException(s"Error when saving ${C.ORDENCOMPRAAPLIC}")
}
...
def load(user: CompanyUser, id: Int): Option[Ordencompraaplic] = {
    loadWhere(user, s"${C.ID} = {id}", 'id -> id)
}

def loadWhere(user: CompanyUser, where: String, args : scala.Tuple2[scala.Any, anorm.ParameterValue[_]]*) = {
    DB.withConnection(user.database.database) { implicit connection =>
    SQL(s"SELECT t1.*, t2.${C.FK_NAME} FROM ${C.ORDENCOMPRAAPLIC} t1 INNER JOIN ${C.???} t2 ON t1.${C.FK_ID} = t2.${C.FK_ID} WHERE $where")
.on(args: _*)
.as(ordencompraaplicParser.singleOpt)
}
}
...
def delete(user: CompanyUser, id: Int) = {
    DB.withConnection(user.database.database) { implicit connection =>
    try {
        SQL(s"DELETE FROM ${C.ORDENCOMPRAAPLIC} WHERE ${C.ID} = {id}")
    .on('id -> id)
            .executeUpdate
    } catch {
    case NonFatal(e) => {
            Logger.error(s"can't delete a ${C.ORDENCOMPRAAPLIC}. ${C.ID} id: $id. Error ${e.toString}")
            throw e
        }
    }
}
...
def get(user: CompanyUser, id: Int): Ordencompraaplic = {
    load(user, id) match {
        case Some(p) => p
    case None => emptyOrdencompraaplic
}
}
}
...
GET     /api/v1/general/ordencompraaplic/:id              controllers.logged.modules.general.Ordencompraaplics.get(id: Int)
POST    /api/v1/general/ordencompraaplic                  controllers.logged.modules.general.Ordencompraaplics.create
PUT     /api/v1/general/ordencompraaplic/:id              controllers.logged.modules.general.Ordencompraaplics.update(id: Int)
DELETE  /api/v1/general/ordencompraaplic/:id              controllers.logged.modules.general.Ordencompraaplics.delete(id: Int)
