  ...
  Cairo.module("CentroCosto.Edit", function(Edit, Cairo, Backbone, Marionette, $, _) {
    ...
    var createObject = function() {
      ...
      self.save = function() {
        ...
        for(var _i = 0; _i < _count; _i++) {
          ...
          switch (property.getKey()) {
            case K_NAME:
              fields.add(C.CCOS_NAME, property.getValue(), Types.text);
              break;

            case K_CODE:
              fields.add(C.CCOS_CODE, property.getValue(), Types.text);
              break;
            ...
            case K_COMPRA:
              fields.add(C.CCOS_COMPRA, Cairo.Util.val(property.getValue()), Types.boolean);
              break;

            case K_VENTA:
              fields.add(C.CCOS_VENTA, Cairo.Util.val(property.getValue()), Types.boolean);
              break;

            case K_CCOS_ID_PADRE:
              fields.add(C.CCOS_ID_PADRE, property.getSelectId(), Types.id);
              break;
          }
        }
      ...
      var load = function(id) {
        ...
        return DB.getData("load[" + m_apiPath + "general/centrodecosto]", id).then(
          function(response) {
