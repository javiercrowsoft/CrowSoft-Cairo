(function() {
  ...
  Cairo.module("XxxxListDoc.Edit", function(Edit, Cairo, Backbone, Marionette, $, _) {
    ...
    var createObject = function() {
      ...
      var C_MODULE = "cOrdenServicioListDoc";
      ...
      self.getSearchParamTable = function() {
        return Cairo.Tables.PRODUCTOSERIE;
      };
      ...
      self.setSearchParam = function(id,  name) {
        ...
        var abmGen = null;
        abmGen = m_dialog;
        abmGen.ShowValue(m_dialog.getProperties().item(mTicketConstantes.PRNS_ID));
      };
        ...
        try {
          ...
          switch (index) {
            ...
            case m_menuShowRemitoAuto:
              pShowRemito(true, false, false, Cairo.Constants.NO_ID, false);

              break;

            case m_menuShowRemito:
              pShowRemito(false, false, false, Cairo.Constants.NO_ID, false);
      ...
      var load = function(us_id) {
        ...
        return Cairo.Database.getData("load[" + apiPath + "general/ordenserviciolistdoc]", id).then(
          function(response) {
            ...
            if(response.data.id === Cairo.Constants.NO_ID) {
              ...
              strLoad = Cairo.Language.getText(2295, "");
              ...
              if(m_ccos_id.Substring(0, 1).toUpperCase() != KEY_NODO) {
                if(!Cairo.Database.getData(mTicketConstantes.CENTROCOSTO, mTicketConstantes.CCOS_ID, Cairo.Util.val(m_ccos_id), mTicketConstantes.CCOS_NAME, data, C_LoadFunction, C_MODULE, strLoad)) { return false; }
                m_centroCosto = data;
              }
              if(m_suc_id.Substring(0, 1).toUpperCase() != KEY_NODO) {
              ...
              if(m_doc_id.Substring(0, 1).toUpperCase() != KEY_NODO) {
                if(!Cairo.Database.getData(mTicketConstantes.DOCUMENTO, mTicketConstantes.DOC_ID, Cairo.Util.val(m_doc_id), mTicketConstantes.DOC_NAME, data, C_LoadFunction, C_MODULE, strLoad)) { return false; }
                m_documento = data;
              }
              if(m_cpg_id.Substring(0, 1).toUpperCase() != KEY_NODO) {
                if(!Cairo.Database.getData(mTicketConstantes.CONDICIONPAGO, mTicketConstantes.CPG_ID, Cairo.Util.val(m_cpg_id), mTicketConstantes.CPG_NAME, data, C_LoadFunction, C_MODULE, strLoad)) { return false; }
      ...
      var cIABMListDocClient_PropertyChange = function(key) {
        ...
        switch (key) {
          ...
          case K_CCOS_ID:
            var property = properties.item(mTicketConstantes.CCOS_ID);
          ...
          case K_DOC_ID:
            var property = properties.item(mTicketConstantes.DOC_ID);
          ...
          case K_CPG_ID:
            var property = properties.item(mTicketConstantes.CPG_ID);
          ...
          case K_PRNS_ID:
            ...
            m_productoNumeroSerie = property.getValue();
      ...
      var cIABMListDocClient_Save = function() {
        ...
        strError = Cairo.Language.getText(2291, "");
      ...
      var pCreateMenu = function() {
        ...
        //'Remitir Automatico
        m_menuShowRemitoAuto = m_objList.AddMenu(Cairo.Language.getText(3953, ""));
        //'Remitir
        ...
        for (i = 1; i <= (m_vMenuModoPago, 2).Length; i++) {
          if(m_vMenuModoPago[C_VM_MENU_ID, i] == idMenu) {
            ...
            if(m_vMenuModoPago[C_VM_IS_CONTADO, i]) {
              ...
              pShowRemito(true, true, false, Cairo.Constants.NO_ID, true);
            }
          }
        }
      };
      ...
      var pShowMensajes = function() {
        ...
        sqlstmt = "sp_ParteDiarioGetTitleForDoc "+ csEDocumentoTipo.cSEDT_ORDENSERVICIO.toString()+ ","+ osId.toString();
        ...
        doctId = Cairo.Database.valField(rs.getFields(), mTicketConstantes.DOCT_ID);
        infodoc = Cairo.Database.valField(rs.getFields(), "info_doc");

        sqlstmt = "sp_PartesDiarioGetForDoc "+ Cairo.Database.getUserId().toString()+ ","+ cUtil.getEmpId().toString()+ ","+ doctId.toString()+ ","+ osId.toString();
        ShowNotes(Cairo.Language.getText(1842, "", infodoc), sqlstmt);
      ...
      var pAddMensaje = function() {
        ...
        parte.AddParteToDoc(csEDocumentoTipo.cSEDT_ORDENSERVICIO, m_objList.Id, false);
      };
      ...
      var pFirmar = function() {
        ...
        if(osId == Cairo.Constants.NO_ID) { return; }

        var firmado = null;
        var docId = null;

        if(!Cairo.Database.getData(mTicketConstantes.ORDENSERVICIO, mTicketConstantes.OS_ID, osId, mTicketConstantes.OS_FIRMADO, firmado)) { return; }
        if(!Cairo.Database.getData(mTicketConstantes.ORDENSERVICIO, mTicketConstantes.OS_ID, osId, mTicketConstantes.DOC_ID, docId)) { return; }
      ...
      var pShowDocAux = function() {
        ...
        if(osId) {
          ...
          var stId = null;
          if(!Cairo.Database.getData(mTicketConstantes.ORDENSERVICIO, mTicketConstantes.OS_ID, osId, mTicketConstantes.ST_ID, stId)) { return; }
      ...
      var pShowApply = function() {
        ...
        sqlstmt = "select os_total, os_cotizacion, os_nrodoc, os.cli_id, cli_nombre, os.suc_id, os.doc_id, os.doct_id from OrdenServicio os inner join Cliente cli on os.cli_id = cli.cli_id where os_id = "+ osId.toString();

        if(!Cairo.Database.openRs(sqlstmt, rs)) { return; }

        if(rs.isEOF()) { return; }

        total = Cairo.Database.valField(rs.getFields(), mTicketConstantes.OS_TOTAL);
        cotiz = Cairo.Database.valField(rs.getFields(), mTicketConstantes.OS_COTIZACION);
        nroDoc = Cairo.Database.valField(rs.getFields(), mTicketConstantes.OS_NRODOC);
        cliId = Cairo.Database.valField(rs.getFields(), mTicketConstantes.CLI_ID);
        cliente = Cairo.Database.valField(rs.getFields(), mTicketConstantes.CLI_NAME);
        sucId = Cairo.Database.valField(rs.getFields(), mTicketConstantes.SUC_ID);
        docId = Cairo.Database.valField(rs.getFields(), mTicketConstantes.DOC_ID);
        doctId = Cairo.Database.valField(rs.getFields(), mTicketConstantes.DOCT_ID);
      ...
      var pShowRemito = function(bPushVirtualNext,  bAutoFactura,  bAutoPago,  cue_id,  bModoVentaCtaCte) {
        try {
      ...
      var pGetCliId = function() {
        ...
        Cairo.Database.getData(mTicketConstantes.ORDENSERVICIO, mTicketConstantes.OS_ID, osId, mTicketConstantes.CLI_ID, cliId);

        return cliId;
      };

      var pGetOsIds = function() {
