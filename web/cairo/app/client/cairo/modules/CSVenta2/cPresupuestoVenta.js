(function() {
  ...
  Cairo.module("XxxxListDoc.Edit", function(Edit, Cairo, Backbone, Marionette, $, _) {
    ...
    var createObject = function() {
      ...
      // cPresupuestoVtaListDoc
      // 24-09-06
      //

      var C_MODULE = "cPresupuestoVtaListDoc";
      ...
      var loadCollection = function() {
        ...
        c = m_dialog.getProperties().add(null, mVentaConstantes.CCOS_ID);
        c.setType(Dialogs.PropertyType.select);
        ...
        c = m_dialog.getProperties().add(null, mVentaConstantes.SUC_ID);
        c.setType(Dialogs.PropertyType.select);
        ...
        c = m_dialog.getProperties().add(null, mVentaConstantes.DOC_ID);
        c.setType(Dialogs.PropertyType.select);
        ...
        c = m_dialog.getProperties().add(null, mVentaConstantes.CPG_ID);
        c.setType(Dialogs.PropertyType.select);
      ...
      var load = function(us_id) {
        ...
        return Cairo.Database.getData("load[" + apiPath + "general/presupuestoventalistdoc]", id).then(
          function(response) {
            ...
            if(response.data.id === Cairo.Constants.NO_ID) {
              ...
              //'Error al carga los párametros de navegación de Presupuestos de Venta
              strLoad = Cairo.Language.getText(2224, "");
      ...
      var cIABMListDocClient_PropertyChange = function(key) {
        ...
        switch (key) {
          ...
          case K_CCOS_ID:
            var property = m_dialog.getProperties().item(mVentaConstantes.CCOS_ID);
          ...
          case K_SUC_ID:
            var property = m_dialog.getProperties().item(mVentaConstantes.SUC_ID);
          ...
          case K_VEN_ID:
            var property = m_dialog.getProperties().item(mVentaConstantes.VEN_ID);
          ...
          case K_DOC_ID:
            var property = m_dialog.getProperties().item(mVentaConstantes.DOC_ID);
          ...
          case K_CPG_ID:
            var property = m_dialog.getProperties().item(mVentaConstantes.CPG_ID);
      ...
      var cIABMListDocClient_Refresh = function() {
        ...
        sqlstmt = "sp_lsdoc_PresupuestosVenta ";
      ...
      var cIABMListDocClient_Save = function() {
        ...
        //'Error al grabar los párametros de navegación de Presupuestos de Venta
        strError = Cairo.Language.getText(2224, "");
        ...
        sqlstmt = "delete ListaDocumentoParametro where pre_id = "+ csVentasPrestacion.cSPREVTALISTPRESUPUESTO.toString()+ " and us_id = "+ m_us_id+ " and (emp_id is null or emp_id = "+ cUtil.getEmpId().toString()+ ")";
      ...
      var pGetDocFilter = function() {
        return "'{emp_id=0}doct_id = "+ csEDocumentoTipo.cSEDT_PRESUPUESTOVENTA.toString()+ " or doct_id = "+ csEDocumentoTipo.cSEDT_DEVOLUCIONPRESUVTA.toString()+ "'";
      };
      ...
      var pCreateMenu = function() {
        ...
        m_objList.ClearMenu;
        //'Crear Pedido
        m_menuShowPedido = m_objList.AddMenu(Cairo.Language.getText(1710, ""));
        m_objList.AddMenu("-");
        //'Firmar
      ...
      var pShowMensajes = function() {
        ...
        sqlstmt = "sp_ParteDiarioGetTitleForDoc "+ csEDocumentoTipo.cSEDT_PRESUPUESTOVENTA.toString()+ ","+ prvId.toString();
        ...
        sqlstmt = "sp_PartesDiarioGetForDoc "+ Cairo.Database.getUserId().toString()+ ","+ cUtil.getEmpId().toString()+ ","+ doctId.toString()+ ","+ prvId.toString();
        ShowNotes(Cairo.Language.getText(1711, "", infodoc), sqlstmt);
      ...
      var pAddMensaje = function() {
        ...
        parte.AddParteToDoc(csEDocumentoTipo.cSEDT_PRESUPUESTOVENTA, m_objList.id, false);
      };
      ...
      var pFirmar = function() {
        ...
        if(prvId == Cairo.Constants.NO_ID) { return; }

        var firmado = null;
        var docId = null;

        if(!Cairo.Database.getData(mVentaConstantes.PRESUPUESTOVENTA, mVentaConstantes.PRV_ID, prvId, mVentaConstantes.PRV_FIRMADO, firmado)) { return; }
        if(!Cairo.Database.getData(mVentaConstantes.PRESUPUESTOVENTA, mVentaConstantes.PRV_ID, prvId, mVentaConstantes.DOC_ID, docId)) { return; }
        ...
        sqlstmt = "sp_DocPresupuestoVentaFirmar "+ prvId.toString()+ ","+ us_id.toString();
        if(!Cairo.Database.openRs(sqlstmt, rs)) { return; }

        m_objList.sqlstmt = "sp_lsdoc_PresupuestoVenta";

        m_objList.RefreshLine(prvId);
      ...
      var pShowApply = function() {
        ...
        total = Cairo.Database.valField(rs.getFields(), mVentaConstantes.PRV_TOTAL);
        nroDoc = Cairo.Database.valField(rs.getFields(), mVentaConstantes.PRV_NRODOC);
        ...
        if(!DoCairo.Security.anAccess(csVentasPrestacion.cSPREVTAMODIFYAPLIC, m_doc_id, csE_DocTypePrestacion.cSEDOCTPREAPLICAR)) { return; }

        if(m_objApply == null) {
          m_objApply = new cPresupuestoVentaAplic();

          // Edit Apply
          //
        }
        else {
          if(m_objApply.getId() != prvId) {
            m_objApply = new cPresupuestoVentaAplic();
          }
        }

        if(!m_objApply.self.show(prvId, total * (cotiz != 0) ? cotiz : 1), nroDoc, cliId, cliente, sucId, docId, doctId == csEDocumentoTipo.cSEDT_DEVOLUCIONPRESUVTA)) {
          m_objApply = null;
        }

      };

      var pShowPedido = function() {
        try {

          var o = null;
          o = CSKernelClient2.CreateObject("CSPedidoVenta2.cPedidoVenta");

          o.ShowPedido(pGetCliId(), pGetPrvIds());

          // **TODO:** goto found: GoTo ExitProc;
        }
        catch (ex) {
          Cairo.manageErrorEx(ex.message, "pShowPedido", C_MODULE, "");
          // **TODO:** label found: ExitProc:;
        }
        // **TODO:** on error resume next found !!!
      };
      ...
      var pGetCliId = function() {
        ...
        prvId = m_objList.id;
        Cairo.Database.getData(mVentaConstantes.PRESUPUESTOVENTA, mVentaConstantes.PRV_ID, prvId, mVentaConstantes.CLI_ID, cliId);

        return cliId;
      };

      var pGetPrvIds = function() {
      ...
      self.initialize = function() {
        try {
          ...
          m_title = Cairo.Language.getText(1708, "");
