        ...
        return DB.getData("load[" + m_apiPath + "general/stockclientelistdoc]", id).then(
          function(response) {
            ...
            if(response.data.id === NO_ID) {
              ...
              if(m_cliId.Substring(0, 1).toUpperCase() != KEY_NODO) {
                if(!DB.getData(mStockConstantes.CLIENTE, mStockConstantes.CLI_ID, Cairo.Util.val(m_cliId), mStockConstantes.CLI_NAME, data, C_LoadFunction, C_MODULE, strLoad)) { return false; }
                m_cliente = data;
              }
              if(m_sucId.Substring(0, 1).toUpperCase() != KEY_NODO) {
