  ...
  Cairo.module("XxxxListDoc.Edit", function(Edit, Cairo, Backbone, Marionette, $, _) {
    ...
    var createObject = function() {
        ...
        return DB.getData("load[" + m_apiPath + "general/depositocuponlistdoc]", id).then(
          function(response) {
            ...
            if(response.data.id === NO_ID) {
              ...
              if(m_tjc_id.Substring(0, 1).toUpperCase() != KEY_NODO) {
                if(!DB.getData(mTesoreriaConstantes.TARJETACREDITO, mTesoreriaConstantes.TJC_ID, Cairo.Util.val(m_tjc_id), mTesoreriaConstantes.TJC_NAME, data, C_LoadFunction, C_MODULE, strLoad)) { return false; }
                m_tarjeta = data;
              }
              // empid
              if(m_empId.Substring(0, 1).toUpperCase() != KEY_NODO) {
      ...
      var pFirmar = function() {
        ...
        var dcupId = null;
        dcupId = m_dialog.getId();

        if(dcupId == NO_ID) { return; }

        var firmado = null;
        var docId = null;

        if(!DB.getData(mTesoreriaConstantes.DEPOSITOBANCO, mTesoreriaConstantes.DCUP_ID, dcupId, mTesoreriaConstantes.DCUP_FIRMADO, firmado)) { return; }
        if(!DB.getData(mTesoreriaConstantes.DEPOSITOBANCO, mTesoreriaConstantes.DCUP_ID, dcupId, mTesoreriaConstantes.DOC_ID, docId)) { return; }
      ...
      var pShowAsiento = function() {
        ...
        if(dcupId) {
          ...
          var asId = null;
          if(!DB.getData(mTesoreriaConstantes.DEPOSITOCUPON, mTesoreriaConstantes.DCUP_ID, dcupId, mTesoreriaConstantes.AS_ID, asId)) { return; }
