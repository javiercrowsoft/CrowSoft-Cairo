  ...
  Cairo.module("XxxxListDoc.Edit", function(Edit, Cairo, Backbone, Marionette, $, _) {
    ...
    var createObject = function() {
        ...
        return DB.getData("load[" + m_apiPath + "general/cobranzalistdoc]", id).then(
          function(response) {
            ...
            if(response.data.id === NO_ID) {
              ...
              if(m_cob_id.Substring(0, 1).toUpperCase() != KEY_NODO) {
                if(!DB.getData(mTesoreriaConstantes.COBRADOR, mTesoreriaConstantes.COB_ID, Cairo.Util.val(m_cob_id), mTesoreriaConstantes.COB_NAME, data, C_LoadFunction, C_MODULE, strLoad)) { return false; }
                m_cobrador = data;
              }
              if(m_docId.Substring(0, 1).toUpperCase() != KEY_NODO) {
      ...
      var pFirmar = function() {
        ...
        if(!DB.getData(mTesoreriaConstantes.COBRANZA, mTesoreriaConstantes.COBZ_ID, cobzId, mTesoreriaConstantes.COBZ_FIRMADO, firmado)) { return; }
        if(!DB.getData(mTesoreriaConstantes.COBRANZA, mTesoreriaConstantes.COBZ_ID, cobzId, mTesoreriaConstantes.DOC_ID, docId)) { return; }
      ...
      var pShowAsiento = function() {
        ...
        if(cobzId) {
          ...
          var asId = null;
          if(!DB.getData(mTesoreriaConstantes.COBRANZA, mTesoreriaConstantes.COBZ_ID, cobzId, mTesoreriaConstantes.AS_ID, asId)) { return; }
